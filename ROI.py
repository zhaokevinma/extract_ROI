...
def image_to_numpyarr(image_path):
    ...
    if image.mode == 'RGB':  # This conditional block is to get 'channels' 'from image.mode'
        ...
        return None
    pixel_values = np.array(pixel_values).reshape((width, height, channels))  # Reshape List[tuples] to numpy.ndarray
    return pixel_values
...
def isRed(pixel):
    ...
    green = pixel[1]
    blue = pixel[2]
    if red - green > 50 and red - blue > 50:  # There is more red in a red pixel 
        return True
